...
#define JJPlayerkBaseFile [NSHomeDirectory() stringByAppendingPathComponent:@"Documents"]
#define JJPlayerKVideoFile [NSString stringWithFormat:@"%@/%@",JJPlayerkBaseFile,@"video"]
...
@interface JJPlayerView ()<JJPlayerMaskViewDelegate,UIGestureRecognizerDelegate,AVAssetResourceLoaderDelegate>
{
   id _itemEndObserver;
}
...
- (void)updateConfigure{
    ...
    [self.playerMaskView.loadingView updateWithConfigure:^(JJAnimationConfigure * _Nonnull configure) {
        __strong typeof(self) strongSelf = weakSelf;
        configure.backgroundColor = strongSelf.playerConfigure.strokeColor;
    }];
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context{
    if ([keyPath isEqualToString:@"status"]) {
        ...
        if (self.player.currentItem.status == AVPlayerItemStatusReadyToPlay) {//加载完成,可以播放
            ...
            // 解析失败(播放失败)
            NSLog(@"播放失败：%@,URL:%@",self.player.currentItem.error,_url);
