...
- (void)userContentController:(WKUserContentController *)userContentController didReceiveScriptMessage:(WKScriptMessage *)message replyHandler:(void (^)(id _Nullable, NSString * _Nullable))replyHandler {
    if ([message.name isEqualToString:ZLJSBridgeName])
    {
        if ([message.body isKindOfClass:NSDictionary.class] && message.body[@"params"]) {
            NSDictionary *params = message.body[@"params"];
            NSString *plugin = [params valueForKey:@"plugin"];
            NSString *funcName = [params valueForKey:@"func"];
            NSDictionary *params1 = [params valueForKey:@"params"];
            NSString *callBackID = [params valueForKey:@"callBackID"];
            replyHandler(@"这是一段返回值",nil);
            [self interactWithPlugin:plugin funcName:funcName params:params1 callBackID:callBackID];
        } else {
            NSString *plugin = [message.body valueForKey:@"plugin"];
            NSString *funcName = [message.body valueForKey:@"func"];
            NSDictionary *params = [message.body valueForKey:@"params"];
